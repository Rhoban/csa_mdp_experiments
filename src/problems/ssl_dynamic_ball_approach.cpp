...
static Eigen::Matrix<double, 3, 3> getR2FromR1(const Eigen::Vector2d& p, double alpha)
{
...
SSLDynamicBallApproach::SSLDynamicBallApproach()
  ...
  , target_max_dist(2.0)
  , max_robot_speed(0.8)
  ,  // Working well: 0.8 [m/s]
  max_robot_speed_theta(1.5)
  ...
  max_acc_theta(max_robot_speed_theta / 2)
  ,  // 2 s to reach full speed
  // Finish
  finish_x_limits(0.11, 0.2)
  , finish_y_tol(0.08)
  , finish_diff_speed_x_limits(-0.05, 0.2)
  ,  // No real idea
  ...
  , kick_y_tol(0.02)
  ,  // Kicker width ~0.08
  kick_diff_speed_x_limits(0.0, 0.05)
  ,  // No real idea
...
void SSLDynamicBallApproach::updateLimits()
{
  ...
  state_limits <<
      // Ball Position
      -ball_max_dist,
      ball_max_dist, -ball_max_dist, ball_max_dist,
      // Target Position
      -target_max_dist, target_max_dist, -target_max_dist, target_max_dist,
      // Robot speed
      -max_robot_speed, max_robot_speed, -max_robot_speed, max_robot_speed, -max_robot_speed_theta,
      max_robot_speed_theta,
      // Ball speed
                                                     ...
                                                     std::default_random_engine* engine) const
{
  ...
  if (action.rows() != 4)
  {
    ...
    oss << "SSLDynamicBallApproach::getSuccessor: "
        << " invalid dimension for action, expecting 4 and received " << action.rows();
...
bool SSLDynamicBallApproach::isKickable(const Eigen::VectorXd& state) const
{
  ...
  bool speed_x_ok = speed_diff(0) >= kick_diff_speed_x_limits(0) && speed_diff(0) <= kick_diff_speed_x_limits(1);
...
bool SSLDynamicBallApproach::isOutOfSpace(const Eigen::VectorXd& state) const
{
  ...
  return ball_dist_ko || target_dist_ko || robot_cart_speed_ko || robot_theta_speed_ko || ball_speed_ko || kick_tol_ko;
}
